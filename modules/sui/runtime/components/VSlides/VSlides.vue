...
// Utilities
import { computed, provide, ref, shallowRef, watch, onMounted, onUnmounted } from 'vue';
...
function startAutoplay() {
  stopAutoplay();
  // guard: don't start if there's nowhere to go
  if (!props.autoplay || group.items.value.length < 2) return;
  autoplayTimer.value = window.setInterval(() => {
    // if at end and not continuous, stop
    if (!props.continuous && !canMoveForward.value) {
      stopAutoplay();
    } else {
      next();
    }
  }, Number(props.autoplayInterval));
}
...
// reset on manual nav so interval restarts
function resetAutoplay() {
  if (props.autoplay) startAutoplay();
}

// start/stop when prop changes or on mount/unmount
watch(
  () => props.autoplay,
  (val) => (val ? startAutoplay() : stopAutoplay())
);
watch(
